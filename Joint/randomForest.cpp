...
void RandomForest::Train(
	...
	for (int i = 0; i < numTrees; i++) {
		for (int j = 0; j < numLandmarks; j++) {
			...
			trees[i][j].Train(tempSamples, meanShape, stages, j);

			// when training of rfs[i][j] is finished 
			// all of the training samples go through the random tree for 
			// classification and regression.
			for (int l = 0; l < samples.size(); l++) {
				trees[i][j].nodes[0].sample_idx.push_back(i);
			}

			for (int l = 0; l < trees[i][j].numNodes; l++) {

				// only internal node has children
				if (!trees[i][j].nodes[l].isLeaf) {
					Node &node = trees[i][j].nodes[l];
					for (int m = 0; m < node.sample_idx.size(); m++) {
						...
						double x1 = node.feat[0].x * trees[i][j].radioRadius;
						double y1 = node.feat[0].y * trees[i][j].radioRadius;
						double x2 = node.feat[1].x * trees[i][j].radioRadius; 
						double y2 = node.feat[1].y * trees[i][j].radioRadius;
						...
						if (difference < node.threshold) {
							...
							trees[i][j].nodes[node.cNodesID[0]].sample_idx.push_back(m);
						}
						else {
							// put this ample into its fight child
							trees[i][j].nodes[node.cNodesID[1]].sample_idx.push_back(m);
						}
					}
			...
			vector<int> &nodeID = trees[i][j].leafID;
			for (int l = 0; l < nodeID.size(); l++) {
				Node& temp = trees[i][j].nodes[nodeID[l]];
				double positive = 0;
				double negative = 0;
